...
function parser () {
	function testForReservedCharacters(input, errorMsg = "Reserved character in unquoted string", ...etc) {
		...
		if(testFailed) {
			...
			throw new SyntaxError(`${errorMsg}: "${pre}>>> ${error} <<<${post}"`);
		}
	}
	...
	function separateEnclosedSections (input, offset, regex, marker, errorMsg, ...enclosers) {
		...
		return { enclosures, output: input };
	}
	...
	function isolateFunctions(input, inputParentheticals, marker = "?") {
		...
		while(inputParentheticals.length > 0) {
			...
			while(m = p.match(regex)) {
				...
				const {output, enclosures} = separateBrackets(parentheticals[associatedParenthetical], offset);
				// Save functions
		...
		return { functions, parentheticals, output };
	}
	...
	function seekForward(array, position) {
		...
		while(position < max && array[position] === undefined) {
			position++;
		}
	...
	function seekBackward(array, position) {
		// Returns the first element in the array at `position` or higher
		while(position <= 0 && array[position] === undefined) {
			position--;
		}
		return [array[position], position];
	}
	...
	function evaluate (text, FUNCTIONS) {
		...
		const final = parentheticals.length;
		const output = [];
		strings.forEach((stringText, index) => {
			...
			if(ranks[7]) {
				...
				ranks[7].forEach(pos => {
					...
					let [first, behind] = seekBackward(tokens, pos - 1);
					let [last, ahead] = seekForward(tokens, pos + 1);
					token[pos] = addition ? first + last : first - last;
					delete tokens[behind];
					delete tokens[ahead];
				});
			}
			...
			if(ranks[9]) {
				...
				ranks[9].forEach(pos => {
					...
					let [first, behind] = seekBackward(tokens, pos - 1);
					let [last, ahead] = seekForward(tokens, pos + 1);
					token[pos] = equals ? first === last : first !== last;
					delete tokens[behind];
					delete tokens[ahead];
				});
			}
